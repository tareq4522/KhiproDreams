...
public class RCC_AICarController : MonoBehaviour {
	...
	public bool useRaycasts = false;		//	Using forward and sideways raycasts to avoid obstacles.
	...
	void Navigation (){
		...
		case AIType.FollowWaypoints:
			...
			RCC_Waypoint nextWaypoint = waypointsContainer.waypoints [currentWaypoint];

			// Setting destination of the Navigator. 
			if(navigator.isOnNavMesh)
				navigator.SetDestination (nextWaypoint.transform.transform.position);

			// Checks for the distance to next waypoint. If it is less than written value, then pass to next waypoint.
			if (Vector3.Distance(transform.position, nextWaypoint.transform.position) < (nextWaypointPassRadius / 2f)) {
	...
	void FeedRCC(){
		...
		if(carController.direction == 1){
			if(!limitSpeed){
				...
				carController.throttleInput = gasInput * Mathf.Clamp01(Mathf.Lerp(10f, 0f, (carController.speed) / maximumSpeed));
			}
		}else{
	...
	void OnTriggerExit (Collider col){
		...
		// Checking if vehicle is in any brake zone on scene.
		if (col.gameObject.GetComponentInParent<RCC_AIBrakeZone> ()) {
