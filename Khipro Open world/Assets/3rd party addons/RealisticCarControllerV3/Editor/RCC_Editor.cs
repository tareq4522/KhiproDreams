...
//			EditorGUILayout.PropertyField(serializedObject.FindProperty("autoReverse"), new GUIContent("Auto Reverse", "Enables/Disables auto reversing when player press brake button. Useful for if you are making parking style game."));
			...
			EditorGUILayout.PropertyField(serializedObject.FindProperty("gearShiftUpRPM"), new GUIContent("Gear Shift Up RPM", "Gear Shifting RPM"), false);
...
//		//	Checking if no any steering wheel.
//		if (carScript.FrontLeftWheelCollider && carScript.FrontRightWheelCollider && carScript.RearLeftWheelCollider && carScript.RearRightWheelCollider) {
//
//			if (!carScript.FrontLeftWheelCollider.canSteer && !carScript.FrontRightWheelCollider.canSteer && !carScript.RearLeftWheelCollider.canSteer && !carScript.RearRightWheelCollider.canSteer) {
//
//				if (EditorUtility.DisplayDialog ("No steering wheels found", "No steering wheels found. Making front wheels as steering wheels now.", "Ok")) {
//					
//					carScript.FrontLeftWheelCollider.canSteer = true;
//					carScript.FrontRightWheelCollider.canSteer = true;
...
//			}
//
//			//	Checking if no any brake wheel.
//			if (!carScript.FrontLeftWheelCollider.canBrake && !carScript.FrontRightWheelCollider.canBrake && !carScript.RearLeftWheelCollider.canBrake && !carScript.RearRightWheelCollider.canBrake) {
//
//				if (EditorUtility.DisplayDialog ("No braking wheels found", "No braking wheels found. Making rear wheels as braking wheels now.", "Ok")) {
//
//					carScript.FrontLeftWheelCollider.canBrake = true;
//					carScript.FrontRightWheelCollider.canBrake = true;
//					carScript.RearLeftWheelCollider.canBrake = true;
//					carScript.RearRightWheelCollider.canBrake = true;
//
//					carScript.FrontLeftWheelCollider.brakingMultiplier = 1f;
//					carScript.FrontRightWheelCollider.brakingMultiplier = 1f;
//					carScript.RearLeftWheelCollider.brakingMultiplier = .5f;
//					carScript.RearRightWheelCollider.brakingMultiplier = .5f;
//					 
//				}
//
//			}
//
//			//	Checking if no any handbrake wheel.
//			if (!carScript.FrontLeftWheelCollider.canHandbrake && !carScript.FrontRightWheelCollider.canHandbrake && !carScript.RearLeftWheelCollider.canHandbrake && !carScript.RearRightWheelCollider.canHandbrake) {
//
//				if (EditorUtility.DisplayDialog ("No handbraking wheels found", "No handbraking wheels found. Making rear wheels as handbraking wheels now.", "Ok")) {
//
//					carScript.RearLeftWheelCollider.canHandbrake = true;
//					carScript.RearRightWheelCollider.canHandbrake = true;
//
//				}
//
//			}
